...
home=hero('A little of your time.<br><em>A world of possibility.</em>','Explore a future where everyday care, curiosity and community contribution help more people live well. Choose something you enjoy. See where it leads.','home',True,'A community invitation from Minjerribah')
...
if args.batch>1:
    ...
    for p in projects:body+=f'<article class="glow-card project-card"><p class="status">{esc(p.get("theme","Connected project"))} · {esc(p.get("status","Proposal"))}</p><h3>{esc(p["title"])}</h3><p>{esc(p["description"])}</p><a href="{esc(p["url"])}">Explore project ↗</a></article>'
    ...
    corrections=source.get('corrections',[]) if isinstance(source,dict) else []
    body=hero('Read the archive.<br>Keep the meaning clear.','Earlier documents trace the development of the idea. This guide preserves the current plan’s corrections so older assumptions do not become new promises.','references')
